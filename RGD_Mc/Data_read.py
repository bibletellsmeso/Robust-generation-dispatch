...
class data_read:
	def __init__(self):
		...
		self.kpx_PV_data = pd.read_csv('/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/data/KPX_PV.csv', sep=',', names=['Source', 'Location', 'Date', 'Hour', 'Power'], dtype={'Date': str, 'Hour': str, 'Power': str}, encoding='CP949')[1:] # dtype = DataFrame
		self.kpx_PV_data = pd.DataFrame(self.kpx_PV_data, columns=['Hour', 'Power']).to_numpy(dtype=np.float32)

		self.kpx_load = pd.read_csv('/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/data/KPX_Load.csv', sep=',', names=['Date', 'Load_1', 'Load_2', 'Load_3', 'Load_4', 'Load_5', 'Load_6', 'Load_7', 'Load_8', 'Load_9', 'Load_10', 'Load_11',
		...
		self.PV_pred = np.array(pd.read_csv('/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/data/PV_for_scheduling.txt', names=['PV']), dtype=np.float32)[:,0]
		self.PV_pred[self.PV_pred < 0] = 0

		self.load_pred = np.array(pd.read_csv('/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/data/Load_for_scheduling.txt', names=['Load']), dtype=np.float32)[:,0]
