...
class CCG_SP():
    ...
    def create_model(self):
        ...
        x_segments = 10
        y_segments = 10
        phi_PV_bounds = [[- self.M, self.M] for _ in self.t_set]
        phi_cut_bounds = [[- self.M, 0] for _ in self.t_set]
        phi_load_bounds = [[- self.M, self.M] for _ in self.t_set]
        epsilon_bounds = [[0, 1] for _ in self.t_set]
        delta_bounds = [[0, 1] for _ in self.t_set]

        phi_PV_segment = [[0] * 11  for _ in self.t_set]
        phi_cut_segment = [[0] * 11  for _ in self.t_set]
        phi_load_segment = [[0] * 11  for _ in self.t_set]
        epsilon_pos_segment = [[0] * 11  for _ in self.t_set]
        epsilon_neg_segment = [[0] * 11  for _ in self.t_set]
        delta_pos_segment = [[0] * 11  for _ in self.t_set]
        delta_neg_segment = [[0] * 11  for _ in self.t_set]

        phi_PV_n = model.addVars(self.nb_periods, x_segments, lb=-self.M, ub=self.M, vtype=GRB.CONTINUOUS, obj=0, name="phi_PV") # free of dual variable
        phi_load = model.addVars(self.nb_periods, lb=-self.M, ub=self.M, vtype=GRB.CONTINUOUS, obj=0, name="phi_load") # free of dual variable
        phi_cut = model.addVars(self.nb_periods, lb=-self.M, ub=0, vtype=GRB.CONTINUOUS, obj=0, name="phi_cut")

        for i in self.t_set:
            for k in range(x_segments + 1):
                phi_PV_segment[i][k] = phi_PV_bounds[i][0] + k * (phi_PV_bounds[i][1] - phi_PV_bounds[i][0]) / x_segments
                phi_cut_segment[i][k] = phi_cut_bounds[i][0] + k * (phi_cut_bounds[i][1] - phi_cut_bounds[i][0]) / x_segments
                phi_load_segment[i][k] = phi_load_bounds[i][0] + k * (phi_load_bounds[i][1] - phi_load_bounds[i][0]) / x_segments
            for l in range(y_segments + 1):
                epsilon_pos_segment[i][l] = epsilon_bounds[i][0] + l * (epsilon_bounds[i][1] - epsilon_bounds[i][0]) / y_segments
                epsilon_neg_segment[i][l] = epsilon_bounds[i][0] + l * (epsilon_bounds[i][1] - epsilon_bounds[i][0]) / y_segments
                delta_pos_segment[i][l] = delta_bounds[i][0] + l * (delta_bounds[i][1] - delta_bounds[i][0]) / y_segments
                delta_neg_segment[i][l] = delta_bounds[i][0] + l * (delta_bounds[i][1] - delta_bounds[i][0]) / y_segments

        # Constraints for piecewise McCormick envelopes
        # Piecewise McCormick constraints for alpha_pos/neg
        model.addConstrs((alpha_pos[i] >= phi_PV[i] * epsilon_pos_segment[i][l] + phi_PV_segment[i][k] * epsilon_pos[i] - phi_PV_segment[i][k] * epsilon_pos_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_pos_GDP1')
        model.addConstrs((alpha_pos[i] >= phi_PV[i] * epsilon_pos_segment[i][l+1] + phi_PV_segment[i][k+1] * epsilon_pos[i] - phi_PV_segment[i][k+1] * epsilon_pos_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_pos_GDP2')
        model.addConstrs((alpha_pos[i] <= phi_PV[i] * epsilon_pos_segment[i][l] + phi_PV_segment[i][k+1] * epsilon_pos[i] - phi_PV_segment[i][k+1] * epsilon_pos_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_pos_GDP3')
        model.addConstrs((alpha_pos[i] <= phi_PV[i] * epsilon_pos_segment[i][l+1] + phi_PV_segment[i][k] * epsilon_pos[i] - phi_PV_segment[i][k] * epsilon_pos_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_pos_GDP4')

        model.addConstrs((alpha_neg[i] >= phi_PV[i] * epsilon_neg_segment[i][l] + phi_PV_segment[i][k] * epsilon_neg[i] - phi_PV_segment[i][k] * epsilon_neg_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_neg_GDP1')
        model.addConstrs((alpha_neg[i] >= phi_PV[i] * epsilon_neg_segment[i][l+1] + phi_PV_segment[i][k+1] * epsilon_neg[i] - phi_PV_segment[i][k+1] * epsilon_neg_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_neg_GDP2')
        model.addConstrs((alpha_neg[i] <= phi_PV[i] * epsilon_neg_segment[i][l] + phi_PV_segment[i][k+1] * epsilon_neg[i] - phi_PV_segment[i][k+1] * epsilon_neg_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_neg_GDP3')
        model.addConstrs((alpha_neg[i] <= phi_PV[i] * epsilon_neg_segment[i][l+1] + phi_PV_segment[i][k] * epsilon_neg[i] - phi_PV_segment[i][k] * epsilon_neg_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_alpha_neg_GDP4')

        # beta_pos/neg
        model.addConstrs((beta_pos[i] >= phi_cut[i] * epsilon_pos_segment[i][l] + phi_cut_segment[i][k] * epsilon_pos[i] - phi_cut_segment[i][k] * epsilon_pos_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_pos_GDP1')
        model.addConstrs((beta_pos[i] >= phi_cut[i] * epsilon_pos_segment[i][l+1] + phi_cut_segment[i][k+1] * epsilon_pos[i] - phi_cut_segment[i][k+1] * epsilon_pos_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_pos_GDP2')
        model.addConstrs((beta_pos[i] <= phi_cut[i] * epsilon_pos_segment[i][l] + phi_cut_segment[i][k+1] * epsilon_pos[i] - phi_cut_segment[i][k+1] * epsilon_pos_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_pos_GDP3')
        model.addConstrs((beta_pos[i] <= phi_cut[i] * epsilon_pos_segment[i][l+1] + phi_cut_segment[i][k] * epsilon_pos[i] - phi_cut_segment[i][k] * epsilon_pos_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_pos_GDP4')

        model.addConstrs((beta_neg[i] >= phi_cut[i] * epsilon_neg_segment[i][l] + phi_cut_segment[i][k] * epsilon_neg[i] - phi_cut_segment[i][k] * epsilon_neg_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_neg_GDP1')
        model.addConstrs((beta_neg[i] >= phi_cut[i] * epsilon_neg_segment[i][l+1] + phi_cut_segment[i][k+1] * epsilon_neg[i] - phi_cut_segment[i][k+1] * epsilon_neg_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_neg_GDP2')
        model.addConstrs((beta_neg[i] <= phi_cut[i] * epsilon_neg_segment[i][l] + phi_cut_segment[i][k+1] * epsilon_neg[i] - phi_cut_segment[i][k+1] * epsilon_neg_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_neg_GDP3')
        model.addConstrs((beta_neg[i] <= phi_cut[i] * epsilon_neg_segment[i][l+1] + phi_cut_segment[i][k] * epsilon_neg[i] - phi_cut_segment[i][k] * epsilon_neg_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_beta_neg_GDP4')

        # gamma_pos/neg
        model.addConstrs((gamma_pos[i] >= phi_load[i] * delta_pos_segment[i][l] + phi_load_segment[i][k] * delta_pos[i] - phi_load_segment[i][k] * delta_pos_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_pos_GDP1')
        model.addConstrs((gamma_pos[i] >= phi_load[i] * delta_pos_segment[i][l+1] + phi_load_segment[i][k+1] * delta_pos[i] - phi_load_segment[i][k+1] * delta_pos_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_pos_GDP2')
        model.addConstrs((gamma_pos[i] <= phi_load[i] * delta_pos_segment[i][l] + phi_load_segment[i][k+1] * delta_pos[i] - phi_load_segment[i][k+1] * delta_pos_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_pos_GDP3')
        model.addConstrs((gamma_pos[i] <= phi_load[i] * delta_pos_segment[i][l+1] + phi_load_segment[i][k] * delta_pos[i] - phi_load_segment[i][k] * delta_pos_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_pos_GDP4')

        model.addConstrs((gamma_neg[i] >= phi_load[i] * delta_neg_segment[i][l] + phi_load_segment[i][k] * delta_neg[i] - phi_load_segment[i][k] * delta_neg_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_neg_GDP1')
        model.addConstrs((gamma_neg[i] >= phi_load[i] * delta_neg_segment[i][l+1] + phi_load_segment[i][k+1] * delta_neg[i] - phi_load_segment[i][k+1] * delta_neg_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_neg_GDP2')
        model.addConstrs((gamma_neg[i] <= phi_load[i] * delta_neg_segment[i][l] + phi_load_segment[i][k+1] * delta_neg[i] - phi_load_segment[i][k+1] * delta_neg_segment[i][l] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_neg_GDP3')
        model.addConstrs((gamma_neg[i] <= phi_load[i] * delta_neg_segment[i][l+1] + phi_load_segment[i][k] * delta_neg[i] - phi_load_segment[i][k] * delta_neg_segment[i][l+1] for i in self.t_set for l in range(y_segments) for k in range(x_segments)), name='c_gamma_neg_GDP4')

        # model.addConstrs((phi_PV_segment[i][k] <= phi_PV[i] for i in self.t_set for k in range(x_segments)), name='c_phi_PV_lb')
        # model.addConstrs((phi_PV_segment[i][k+1] >= phi_PV[i] for i in self.t_set for k in range(x_segments)), name='c_phi_PV_ub')
        # model.addConstrs((phi_cut_segment[i][k] <= phi_cut[i] for i in self.t_set for k in range(x_segments)), name='c_phi_cut_lb')
        # model.addConstrs((phi_cut_segment[i][k+1] >= phi_cut[i] for i in self.t_set for k in range(x_segments)), name='c_phi_cut_ub')
        # model.addConstrs((phi_load_segment[i][k] <= phi_load[i] for i in self.t_set for k in range(x_segments)), name='c_phi_load_lb')
        # model.addConstrs((phi_load_segment[i][k+1] >= phi_load[i] for i in self.t_set for k in range(x_segments)), name='c_phi_load_ub')
        # model.addConstrs((epsilon_pos_segment[i][k] <= epsilon_pos[i] for i in self.t_set for k in range(x_segments)), name='c_epsion_pos_lb')
        # model.addConstrs((epsilon_pos_segment[i][l+1] >= epsilon_pos[i] for i in self.t_set for l in range(y_segments)), name='c_epsilon_pos_ub')
        # model.addConstrs((epsilon_neg_segment[i][l] <= epsilon_neg[i] for i in self.t_set for l in range(y_segments)), name='c_epsilon_neg_lb')
        # model.addConstrs((epsilon_neg_segment[i][l+1] >= epsilon_neg[i] for i in self.t_set for l in range(y_segments)), name='c_epsilon_neg_ub')
        # model.addConstrs((delta_pos_segment[i][l] <= delta_pos[i] for i in self.t_set for l in range(y_segments)), name='c_delta_pos_lb')
        # model.addConstrs((delta_pos_segment[i][l+1] >= delta_pos[i] for i in self.t_set for l in range(y_segments)), name='c_delta_pos_ub')
        # model.addConstrs((delta_neg_segment[i][l] <= delta_neg[i] for i in self.t_set for l in range(y_segments)), name='c_delta_neg_ub')
        # model.addConstrs((delta_neg_segment[i][l+1] >= delta_neg[i] for i in self.t_set for l in range(y_segments)), name='c_delta_neg_lb')

        # for i in self.t_set:
        #     for k in range(x_segments):
                # model.addConstr((phi_PV_segment[i][k] <= phi_PV[i]), name='c_phi_PV_lb')
    ...
    def solve(self, LogToConsole:bool=False, logfile:str="", Threads:int=0, MIPFocus:int=0, TimeLimit:float=GRB.INFINITY):
        ...
        if self.model.status == 2 or self.model.status == 9:
            ...
            print("\nVariable values in the infeasible solution attempt:")
            for v in self.model.getVars():
                print(f"{v.varName}:")
                print(f"  Value: {v.x}")
                print(f"  Lower Bound: {v.lb}")
                print(f"  Upper Bound: {v.ub}")
                print(f"  Objective Coefficient: {v.obj}")
                print(f"  Variable Type: {v.vtype}")
            if self.model.getAttr(GRB.Attr.IISLB, v):
                print(f"  IIS Lower Bound: {v.lb}")
            if self.model.getAttr(GRB.Attr.IISUB, v):
                print(f"  IIS Upper Bound: {v.ub}")
