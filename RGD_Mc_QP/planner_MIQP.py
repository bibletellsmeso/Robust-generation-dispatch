...
class Planner_MIQP():
    ...
    def create_model(self):
        ...
        x_PV_cut = model.addVars(self.nb_periods, lb=0, ub=GRB.INFINITY, obj=0, vtype=GRB.CONTINUOUS, name='x_PV_cut') # pre-dispatch curtailment of RE
        ...
        model.addConstrs((x[i] - x_chg[i] + x_dis[i] + x_PV[i] - x_PV_cut[i] - x_load[i] == 0 for i in self.t_set), name='c_power_balance')

        model.addConstrs((x_PV[i] == self.PV_forecast[i] for i in self.t_set), name='c_PV_output')
        model.addConstrs((x_PV_cut[i] <= self.PV_lb[i] for i in self.t_set), name='c_x_PV_cut')
...
if __name__ == "__main__":
    ...
    PV_trajectory = read_file(dir='/Users/Andrew/OneDrive/Programming/Python/Optimization/Robust generation dispatch/RGD_Mc/export_CCG/PV_Sandia/', name='2018-07-04_PV_worst_case')
    load_trajectory = read_file(dir='/Users/Andrew/OneDrive/Programming/Python/Optimization/Robust generation dispatch/RGD_Mc/export_CCG/PV_Sandia/', name='2018-07-04_load_worst_case')
    PV_lb = data.PV_lb
    PV_ub = data.PV_ub
    ...
    planner = Planner_MIQP(PV_forecast=PV_forecast, load_forecast=load_forecast, PV_trajectory=PV_trajectory, load_trajectory=load_trajectory)
    ...
    dump_file(dir=dirname, name='sol_MILP_curtailment', file=solution['x_PV_cut'])
    ...
    plt.figure(figsize=(16,9))
    plt.plot(solution['x_PV_cut'], label='PV curt predict')
    plt.ylabel('kW', fontsize=FONTSIZE)
    plt.legend(fontsize=FONTSIZE)
    ...
    plt.plot(solution['x_PV_cut'], color='dimgrey', linestyle='-.', linewidth=2, label='PV curtailment')
    plt.plot(([hs - eg for hs, eg in zip(solution['x_PV'], solution['x_PV_cut'])]), linestyle='--', color='darkorchid', linewidth=2, label='PV output')
    plt.legend()
    plt.savefig(dirname + day + 'MILP_day ahead' + '.png')
    plt.close('all')

    plt.figure(figsize=(16,9))
    plt.plot(solution['y_PV_cut'], label='PV curtailment real time')
    plt.plot(solution['y_PV_add'], label='PV addition real time')
    plt.ylabel('kW', fontsize=FONTSIZE)
    plt.legend(fontsize=FONTSIZE)
    ...
    plt.plot(([hs - egg + eg for hs, egg, eg in zip(solution['y_PV'], solution['y_PV_cut'], solution['y_PV_add'])]), linestyle='--', color='darkorchid', linewidth=2, label='PV output')
    plt.legend()
