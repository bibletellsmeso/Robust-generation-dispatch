...
class CCG_SP_QP():
    ...
    def __init__(self, PV_forecast:np.array, load_forecast:np.array, power:np.array, reserve_pos:np.array, reserve_neg:np.array,
                 ...
                 M_PV_best:np.array, M_PV_worst:np.array, M_cut_best:np.array, M_cut_worst:np.array, M_load_best:np.array, M_load_worst:np.array, GAMMA:float=0, PI:float=0):
        """
        Init the planner.
        """
        ...
        self.M_PV_worst = M_PV_worst
        self.M_cut_best = M_cut_best
    ...
    def create_model(self):
        ...
        # 2.2 Convex combination approach
        mu_cut = model.addVars(self.nb_periods, lb=-GRB.INFINITY, ub=0, vtype=GRB.CONTINUOUS, obj=0, name="mu_cut")
        mu_add = model.addVars(self.nb_periods, lb=-GRB.INFINITY, ub=0, vtype=GRB.CONTINUOUS, obj=0, name="mu_add")

        # 2.3 Continuous variables related to the uncertainty set
        ...
        # 2.4 Continuous variables use for the linearization of the bilinear terms
        ...
        # 2.5 Binary variables for uncertainty variable simultaneous
        ...
        for i in self.t_set:
            phi_PV_lb[i], phi_PV_ub[i] = min(self.M_PV_best[i], self.M_PV_worst[i]), max(self.M_PV_best[i], self.M_PV_worst[i])
            phi_cut_lb[i], phi_cut_ub[i] = min(self.M_cut_best[i], self.M_cut_worst[i]), max(self.M_cut_best[i], self.M_cut_worst[i])
            phi_load_lb[i], phi_load_ub[i] = min(self.M_load_best[i], self.M_load_worst[i]), max(self.M_load_best[i], self.M_load_worst[i])

        epsilon_pos_lb, epsilon_pos_ub = [0] * 96, [1] * 96
        epsilon_neg_lb, epsilon_neg_ub = [0] * 96, [1] * 96
        delta_pos_lb, delta_pos_ub = [0] * 96, [1] * 96
        delta_neg_lb, delta_neg_ub = [0] * 96, [1] * 96

        model.addConstrs((- phi_PV[i] <= - phi_PV_lb[i] for i in self.t_set), name='c_phi_PV_lb')
        model.addConstrs((phi_PV[i] <= phi_PV_ub[i] for i in self.t_set), name='c_phi_PV_ub')
        model.addConstrs((- phi_cut[i] <= - phi_cut_lb[i] for i in self.t_set), name='c_phi_cut_lb')
        model.addConstrs((phi_cut[i] <= phi_cut_ub[i] for i in self.t_set), name='c_phi_cut_ub')
        model.addConstrs((- phi_load[i] <= - phi_load_lb[i] for i in self.t_set), name='c_phi_load_lb')
        model.addConstrs((phi_load[i] <= phi_load_ub[i] for i in self.t_set), name='c_phi_load_ub')

        # alpha_pos/neg
        model.addConstrs((alpha_pos[i] >= phi_PV_lb[i] * epsilon_pos[i] + phi_PV[i] * epsilon_pos_lb[i] - phi_PV_lb[i] * epsilon_pos_lb[i] for i in self.t_set), name='c_alpha_pos_1')
        model.addConstrs((alpha_pos[i] >= phi_PV_ub[i] * epsilon_pos[i] + phi_PV[i] * epsilon_pos_ub[i] - phi_PV_ub[i] * epsilon_pos_ub[i] for i in self.t_set), name='c_alpha_pos_2')
        model.addConstrs((alpha_pos[i] <= phi_PV_ub[i] * epsilon_pos[i] + phi_PV[i] * epsilon_pos_lb[i] - phi_PV_ub[i] * epsilon_pos_lb[i] for i in self.t_set), name='c_alpha_pos_3')
        model.addConstrs((alpha_pos[i] <= phi_PV_lb[i] * epsilon_pos[i] + phi_PV[i] * epsilon_pos_ub[i] - phi_PV_lb[i] * epsilon_pos_ub[i] for i in self.t_set), name='c_alpha_pos_4')

        model.addConstrs((alpha_neg[i] >= phi_PV_lb[i] * epsilon_neg[i] + phi_PV[i] * epsilon_neg_lb[i] - phi_PV_lb[i] * epsilon_neg_lb[i] for i in self.t_set), name='c_alpha_neg_1')
        model.addConstrs((alpha_neg[i] >= phi_PV_ub[i] * epsilon_neg[i] + phi_PV[i] * epsilon_neg_ub[i] - phi_PV_ub[i] * epsilon_neg_ub[i] for i in self.t_set), name='c_alpha_neg_2')
        model.addConstrs((alpha_neg[i] <= phi_PV_ub[i] * epsilon_neg[i] + phi_PV[i] * epsilon_neg_lb[i] - phi_PV_ub[i] * epsilon_neg_lb[i] for i in self.t_set), name='c_alpha_neg_3')
        model.addConstrs((alpha_neg[i] <= phi_PV_lb[i] * epsilon_neg[i] + phi_PV[i] * epsilon_neg_ub[i] - phi_PV_lb[i] * epsilon_neg_ub[i] for i in self.t_set), name='c_alpha_neg_4')

        # beta_pos/neg
        model.addConstrs((beta_pos[i] >= phi_cut_lb[i] * epsilon_pos[i] + phi_cut[i] * epsilon_pos_lb[i] - phi_cut_lb[i] * epsilon_pos_lb[i] for i in self.t_set), name='c_beta_pos_1')
        model.addConstrs((beta_pos[i] >= phi_cut_ub[i] * epsilon_pos[i] + phi_cut[i] * epsilon_pos_ub[i] - phi_cut_ub[i] * epsilon_pos_ub[i] for i in self.t_set), name='c_beta_pos_2')
        model.addConstrs((beta_pos[i] <= phi_cut_ub[i] * epsilon_pos[i] + phi_cut[i] * epsilon_pos_lb[i] - phi_cut_ub[i] * epsilon_pos_lb[i] for i in self.t_set), name='c_beta_pos_3')
        model.addConstrs((beta_pos[i] <= phi_cut_lb[i] * epsilon_pos[i] + phi_cut[i] * epsilon_pos_ub[i] - phi_cut_lb[i] * epsilon_pos_ub[i] for i in self.t_set), name='c_beta_pos_4')

        model.addConstrs((beta_neg[i] >= phi_cut_lb[i] * epsilon_neg[i] + phi_cut[i] * epsilon_neg_lb[i] - phi_cut_lb[i] * epsilon_neg_lb[i] for i in self.t_set), name='c_beta_neg_1')
        model.addConstrs((beta_neg[i] >= phi_cut_ub[i] * epsilon_neg[i] + phi_cut[i] * epsilon_neg_ub[i] - phi_cut_ub[i] * epsilon_neg_ub[i] for i in self.t_set), name='c_beta_neg_2')
        model.addConstrs((beta_neg[i] <= phi_cut_ub[i] * epsilon_neg[i] + phi_cut[i] * epsilon_neg_lb[i] - phi_cut_ub[i] * epsilon_neg_lb[i] for i in self.t_set), name='c_beta_neg_3')
        model.addConstrs((beta_neg[i] <= phi_cut_lb[i] * epsilon_neg[i] + phi_cut[i] * epsilon_neg_ub[i] - phi_cut_lb[i] * epsilon_neg_ub[i] for i in self.t_set), name='c_beta_neg_4')

        # gamma_pos/neg
        model.addConstrs((gamma_pos[i] >= phi_load_lb[i] * delta_pos[i] + phi_load[i] * delta_pos_lb[i] - phi_load_lb[i] * delta_pos_lb[i] for i in self.t_set), name='c_gamma_pos_1')
        model.addConstrs((gamma_pos[i] >= phi_load_ub[i] * delta_pos[i] + phi_load[i] * delta_pos_ub[i] - phi_load_ub[i] * delta_pos_ub[i] for i in self.t_set), name='c_gamma_pos_2')
        model.addConstrs((gamma_pos[i] <= phi_load_ub[i] * delta_pos[i] + phi_load[i] * delta_pos_lb[i] - phi_load_ub[i] * delta_pos_lb[i] for i in self.t_set), name='c_gamma_pos_3')
        model.addConstrs((gamma_pos[i] <= phi_load_lb[i] * delta_pos[i] + phi_load[i] * delta_pos_ub[i] - phi_load_lb[i] * delta_pos_ub[i] for i in self.t_set), name='c_gamma_pos_4')

        model.addConstrs((gamma_neg[i] >= phi_load_lb[i] * delta_neg[i] + phi_load[i] * delta_neg_lb[i] - phi_load_lb[i] * delta_neg_lb[i] for i in self.t_set), name='c_gamma_neg_1')
        model.addConstrs((gamma_neg[i] >= phi_load_ub[i] * delta_neg[i] + phi_load[i] * delta_neg_ub[i] - phi_load_ub[i] * delta_neg_ub[i] for i in self.t_set), name='c_gamma_neg_2')
        model.addConstrs((gamma_neg[i] <= phi_load_ub[i] * delta_neg[i] + phi_load[i] * delta_neg_lb[i] - phi_load_ub[i] * delta_neg_lb[i] for i in self.t_set), name='c_gamma_neg_3')
        ...
        self.allvar['mu_cut'] = mu_cut
        self.allvar['mu_add'] = mu_add
    ...
    def store_solution(self):
        ...
        for var in ['phi_pos', 'phi_neg', 'phi_chg', 'phi_dis', 'phi_Smin', 'phi_Smax',
                    'phi', 'phi_PV', 'phi_load', 'phi_cut', 'phi_add', 'mu_cut', 'mu_add',
...
if __name__ == "__main__":
    ...
    SP_dual = CCG_SP_QP(PV_forecast=PV_forecast, load_forecast=load_forecast, power=power, reserve_pos=reserve_pos, reserve_neg=reserve_neg,
                     charge=charge, discharge=discharge, SOC=SOC, curtailment=curtailment,
                     M_PV_best=M_phi_PV_best, M_PV_worst=M_phi_PV_worst, M_cut_best=M_phi_cut_best, M_cut_worst=M_phi_cut_worst, M_load_best=M_phi_load_best, M_load_worst=M_phi_load_worst, GAMMA=GAMMA, PI=PI)
    ...
    plt.ylabel('Power (kW)')
    plt.legend(ncol=1, loc='upper left', frameon=True, fancybox=False, edgecolor='black', framealpha=0.8)
    plt.savefig(dirname + day + '_worst-case.png', dpi=300)
    plt.show()
    # plt.close('all')