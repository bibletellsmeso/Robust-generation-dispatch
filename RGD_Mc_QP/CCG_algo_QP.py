...
from CCG_MP_QP import CCG_MP_QP
from CCG_SP_Mc_QP import CCG_SP_QP
from SP_primal_QP import SP_primal_QP
from planner_MIQP import Planner_MIQP
...
def ccg_algo(dir:str, tol:float, power:np.array, reserve_pos:np.array, reserve_neg:np.array, charge:np.array, discharge:np.array, SOC:np.array, curtailment:np.array,
    ...
    # Building the MP
    MP = CCG_MP_QP(PV_forecast=PV_forecast, load_forecast=load_forecast)
    MP.model.update()
    ...
    while all(i < tol for i in tolerance_list) is not True and iteration < max_iteration:
        ...
        SP_primal = SP_primal_QP(PV_trajectory=PV_worst_case_from_SP, load_trajectory=load_worst_case_from_SP,
        ...
        planner = Planner_MIQP(PV_forecast=PV_forecast, load_forecast=load_forecast, PV_trajectory=PV_worst_case_from_SP, load_trajectory=load_worst_case_from_SP)
        planner.solve()
...
if __name__ == "__main__":
    ...
    planner = Planner_MIQP(PV_forecast=PV_forecast, load_forecast=load_forecast, PV_trajectory=PV_forecast, load_trajectory=load_forecast)
    planner.solve()
    ...
    curtailment_ini = sol_planner_ini['x_PV_cut']
    ...
    SP_dual = CCG_SP_QP(PV_forecast=PV_forecast, load_forecast=load_forecast, power=final_power, reserve_pos=final_reserve_pos, reserve_neg=final_reserve_neg, 
    ...
    SP_primal = SP_primal_QP(PV_trajectory=PV_worst_case, load_trajectory=load_worst_case, power=final_power, reserve_pos=final_reserve_pos, reserve_neg=final_reserve_neg,
                             charge=final_charge, discharge=final_discharge, SOC=final_SOC, curtailment=final_curtailment)
    SP_primal.solve()
    SP_primal_sol = SP_primal.store_solution()
    ...
    # ------------------------------------------------------------------------------------------------------------------
    planner = Planner_MIQP(PV_forecast=PV_forecast, load_forecast=load_forecast, PV_trajectory=PV_worst_case, load_trajectory=load_worst_case)
    planner.solve()
    sol_planner = planner.store_solution()
