...
class SP_primal_QP():
    ...
    def create_model(self):
        ...
        # -------------------------------------------------------------------------------------------------------------
        # 1. create model
        model = gp.Model("SP_primal_QP_gurobi")
...
if __name__ == "__main__":
    ...
    SP_primal = SP_primal_QP(PV_trajectory=PV_trajectory, load_trajectory=load_trajectory, power=power, reserve_pos=reserve_pos, reserve_neg=reserve_neg, 
                             charge=charge, discharge=discharge, SOC=SOC, curtailment=curtailment)
    
    SP_primal.export_model(dirname + 'SP_primal_QP')
    SP_primal.solve()
