...
from CCG_SP_bM_best import CCG_SP_best
...
def ccg_algo(dir:str, tol:float, power:np.array, reserve_pos:np.array, reserve_neg:np.array,
    ...
    while all(i < tol for i in tolerance_list) is not True and iteration < max_iteration:
        ...
        if SP_dual_sol['status'] == 2 or SP_dual_sol['status'] == 9:  # 2 = optimal, 9 = timelimit has been reached
            ...
            if MP_sol['status'] == 3 or MP_sol['status'] == 4:
                ...
                SP_dual = CCG_SP_best(PV_forecast=PV_forecast, load_forecast=load_forecast, PV_pos=PV_pos, PV_neg=PV_neg, load_pos=load_pos, load_neg=load_neg,
    ...
    return power, reserve_pos, reserve_neg, charge, discharge, SOC, curtailment, df_objectives, conv_inf, x_cost_fuel, x_cost_res, x_cost_ESS, x_cost_cut
...
GAMMA = 0 # Budget of uncertainty to specify the number of time periods where PV generation lies within the uncertainty interval: 0: to 95 -> 0 = no uncertainty
PI = 0
M = 1
...
if __name__ == "__main__":
    ...
    # Create folder
    dirname = '/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/RGD_bM/export_best/'
    if PV_Sandia:
        dirname += 'PV_Sandia/'
    ...
    SP_dual = CCG_SP_best(PV_forecast=PV_forecast, load_forecast=load_forecast, power=final_power, reserve_pos=final_reserve_pos, reserve_neg=final_reserve_neg, 
    ...
    np.savetxt('/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/result/bM_phi_best.csv', phi_data, delimiter=',', header='phi_PV,phi_cut,phi_load', comments='', fmt='%.18f')
    ...
    dump_file(dir='/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/result/', name=day + '_bM_phi_PV_best', file=SP_dual_sol['phi_PV'])
    dump_file(dir='/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/result/', name=day + '_bM_phi_cut_best', file=SP_dual_sol['phi_cut'])
    dump_file(dir='/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/result/', name=day + '_bM_phi_load_best', file=SP_dual_sol['phi_load'])
    ...
    plt.plot(([hs + eg for hs, eg in zip(final_power, final_reserve_pos)]), marker='^', markersize=1, zorder=3, linewidth=2, alpha=0.5, label='DE reserve up')
    plt.plot(([hs - eg for hs, eg in zip(final_power, final_reserve_neg)]), marker='v', markersize=1, zorder=3, linewidth=2, alpha=0.5, label='DE reserve down')
    ...
    plt.plot(PV_worst_case, color='mediumblue', alpha=0.8, linestyle="--", markersize=6, zorder=3, linewidth=3, label='PV worst scenario')
    ...
    plt.plot(([shs - hs + egg - eg for shs, hs, egg, eg in zip(final_discharge, final_charge, SP_primal_sol['y_dis'], SP_primal_sol['y_chg'])]), color='gold', markersize=6, zorder=1, linewidth=3, label='ESS_output')
    ...
    np.savetxt('/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/result/best_case_bM.csv', data, delimiter=',', header='PV_best, load_best', comments='', fmt='%.18f')
    ...
    df.to_excel('/Users/PSL/OneDrive/Programing/Python/Robust generation dispatch/result/bM_phi_all_best.xlsx', index=False)